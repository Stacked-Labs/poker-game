...
const StatCard = ({ title, timestamp, flex, children }: { title: string; timestamp: string; flex: number; children?: ReactNode }) => (
    <Flex flex={flex} direction={'column'} bg={'gray.200'} color={'white'} p={6} rounded={'lg'}>
...
const Timestamp = ({ time }: { time: string }) => {
    ...
    return (
        <Text fontSize={'sm'} fontWeight={'light'} color={'grey'} align={'center'}>
            As of {formattedTime}
        </Text>
    )
}
...
const page = async () => {
    ...
    return (
        <Flex direction={'column'} bg={'charcoal.800'} pt={28} pb={10} minHeight={'100vh'} alignItems={'center'} w={'100%'} px={30}>
            <Text fontSize="3xl" fontWeight="bold" mb={6} color={'white'}>Stats</Text>
            <Flex direction={'column'} gap={2} width={{ base: 'full', lg: '70vw' }} pb={{ base: 4, lg: 0 }} >
                <Flex width={'full'} gap={2} direction={{ base: 'column', lg: 'row' }}>
                    <StatCard flex={2} title={"Stacked"} timestamp={debugTables.timestamp}>
                        <StatBody>
                            <Text textTransform={'capitalize'} color={health.status === 'healthy' ? 'green.400' : 'red.500'}>{health.status}</Text>
                            <Flex
                                as="span"
                                w="16px"
                                h="16px"
                    ...
                    <Flex flex={1} width={'full'} direction={'column'} gap={2}>
                        <StatCard flex={1} title={"Database Health"} timestamp={healthDb.timestamp}>
                            <StatBody>
                        ...
                        <StatCard flex={1} title={"Metrics"} timestamp={metrics.timestamp}>
                            <Flex direction={'column'} gap={2}>
                ...
                <Flex width={'full'}>
                    <StatCard flex={1} title={"Debug"} timestamp={debugTables.timestamp}>
                        <StatBody>
