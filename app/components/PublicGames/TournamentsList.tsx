...
function CreateTournamentModal({
    ...
    const guaranteeAmountMicro = guaranteeUsdc
        ? Math.round(parseFloat(guaranteeUsdc) * 1_000_000)
        : 0;
    ...
    const handleCreate = async () => {
        ...
        if (isNaN(startMs) || startMs < Date.now() + 3 * 60 * 1000) {
            toast.warning(
                'Start time must be at least 3 minutes in the future'
            );
            return;
        }
        if (!isFreePlay) {
            ...
            if (buyIn === 0 && guarantee === 0) {
                toast.warning(
                    'Freeroll tournaments require a guarantee — set a prize pool amount'
                );
                return;
            }
        }
        ...
        try {
            ...
            const needsGuarantee =
                !isFreePlay && guaranteeUsdc && parseFloat(guaranteeUsdc) > 0;
            const result = await createTournament({
                name: tournamentName.trim() || undefined,
                min_entries: parseInt(minPlayers, 10),
                max_entries: parseInt(maxPlayers, 10),
                buy_in_usdc: isFreePlay
                    ? 0
                    : Math.round(parseFloat(buyInUsdc) * 1_000_000),
                guarantee_usdc: needsGuarantee
                    ? guaranteeAmountMicro
                    : undefined,
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleClose} isCentered>
            ...
            <ModalContent>
                <ModalHeader>
                    {step === 'fund' ? 'Fund Guarantee' : 'Create Tournament'}
                </ModalHeader>
                <ModalCloseButton />
                <ModalBody>
                    {step === 'fund' && createdTournament ? (
                        <VStack spacing={4} py={2}>
                            <Text
                                ...
                                textAlign="center"
                            >
                                Tournament deployed. Deposit the guarantee so
                                players know the prize pool is secured. Any
                                unused portion is returned to you when the
                                tournament starts.
                            </Text>
                            <Text
                                fontSize="2xl"
                                ...
                                color="brand.green"
                            >
                                ${(guaranteeAmountMicro / 1_000_000).toFixed(0)}{' '}
                                USDC GTD
                            </Text>
                        ...
                        <VStack spacing={4}>
                            ...
                            <FormControl display="flex" alignItems="center">
                                ...
                                <Switch
                                    ...
                                    onChange={(e) =>
                                        setIsFreePlay(e.target.checked)
                                    }
                                    colorScheme="green"
                                />
                            </FormControl>
                            ...
                            <HStack w="full" spacing={3}>
                                <FormControl>
                                    ...
                                    <Select
                                        ...
                                        onChange={(e) =>
                                            setBlindStructure(e.target.value)
                                        }
                                        size="sm"
                                    >
                                        <option value="hyper">
                ...
                <ModalFooter>
                    ...
                    {step === 'fund' ? (
                        <Button
                            ...
                            isLoading={isFunding}
                            loadingText={
                                fundStatusLabel[fundStatus] ?? 'Processing…'
                            }
                            isDisabled={fundStatus === 'success'}
                            onClick={handleFundAndOpen}
                        >
                            Approve &amp; Fund $
                            {(guaranteeAmountMicro / 1_000_000).toFixed(0)} GTD
                        </Button>
                    ) : (
                        <Button
                            colorScheme="green"
